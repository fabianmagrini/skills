...
- This skill pairs naturally with `/create-ticket` (use debug-issue to diagnose a bug, then create-ticket to record the fix scope before implementing), `/review-code` (reviewing the fix before merging), `/write-tests` (adding a regression test for the bug), and `/incident-review` (when the bug caused a production incident and a postmortem is warranted).
