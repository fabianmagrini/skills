...
- This skill pairs naturally with `/create-ticket` (use debug-issue to diagnose a bug, then create-ticket to record the fix scope before implementing), `/explain-codebase` (when debugging in an unfamiliar codebase, a quick explanation of the relevant module helps orient the investigation), `/review-code` (reviewing the fix before merging), `/write-tests` (adding a regression test for the bug), and `/incident-review` (when the bug caused a production incident and a postmortem is warranted).
