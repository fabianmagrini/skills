...
- This skill pairs naturally with `/platform-readiness` (which checks whether a runbook exists as part of its CI/CD pillar), `/incident-review` (which references runbook procedures during postmortems), `/test-strategy` (smoke tests and health checks documented in the runbook should align with the test suite), `/migrate-data` (the runbook's Database and Migrations section should reference the migration plan for any schema changes in the deployment), and `/onboard-codebase` (developer-facing orientation first — runbook covers operational procedures once the engineer is productive).
