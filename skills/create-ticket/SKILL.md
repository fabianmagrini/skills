...
- This skill pairs naturally with `/complete-ticket` (implementing the ticket once it is written), `/spec-to-backlog` (for larger features, generate a full backlog first — then use create-ticket for individual items not captured there), and `/debug-issue` (use debug-issue to diagnose a bug, then create-ticket to record the fix scope before implementing).
