...
- This skill pairs naturally with `/estimate-effort` (estimate the ticket immediately after writing it, before it enters a sprint), `/complete-ticket` (implementing the ticket once it is written), `/spec-to-backlog` (for larger features, generate a full backlog first — then use create-ticket for individual items not captured there), `/debug-issue` (use debug-issue to diagnose a bug, then create-ticket to record the fix scope before implementing), and `/write-changelog` (the full build workflow: create-ticket → complete-ticket → write-changelog).
