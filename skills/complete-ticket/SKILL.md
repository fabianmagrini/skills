...
- This skill pairs naturally with `/create-ticket` (which creates the ticket file that this skill implements), `/estimate-effort` (the estimate surfaces unknowns the implementing engineer will encounter), `/spec-to-backlog` (which generates a full backlog of tickets from a feature spec), `/summarise-pr` (summarise a related PR that touched the same area before implementing, to understand prior context), `/write-changelog` (completed tickets are the primary source of changelog entries — run write-changelog after a batch of tickets are done), and `/write-tests` (for adding test coverage outside the ticket flow).
