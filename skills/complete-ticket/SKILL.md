...
- This skill pairs naturally with `/create-ticket` (which creates the ticket file that this skill implements), `/spec-to-backlog` (which generates a full backlog of tickets from a feature spec), and `/write-tests` (for adding test coverage outside the ticket flow).
