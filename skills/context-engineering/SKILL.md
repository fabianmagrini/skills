...
- This skill pairs naturally with `/agent-loop-design` (the context file governs the permissions and instructions of an autonomous loop), `/skill-generator` (skills registered in the context file should be generated and tested before being listed), `/explain-codebase` (a quick codebase explanation is a useful precursor to writing the context file), and `/research-codebase` (a full research pass produces the comprehensive documentation that the context file distills into agent-facing conventions).
