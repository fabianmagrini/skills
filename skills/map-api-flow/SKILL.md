...
- This skill pairs naturally with `/design-api` (formalizing a discovered API surface into an OpenAPI or GraphQL spec), `/document-api` (if the API already exists without a spec, generate the OpenAPI definition from the mapped routes), `/review-code` (reviewing the code within an identified call chain for correctness or security), and `/onboard-codebase` (map specific flows after a new engineer has their initial orientation).
