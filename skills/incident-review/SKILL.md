...
- This skill pairs naturally with `/generate-runbook` (runbook gaps exposed by the incident should be addressed before the next on-call rotation) and `/platform-readiness` (CAPAs often map directly to readiness gaps — observability, rollback, alerting).
