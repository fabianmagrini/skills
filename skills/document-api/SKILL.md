...
- This skill pairs naturally with `/map-api-flow` (use map-api-flow first to trace the full call chain — document-api then formalises what was found into a spec), `/design-api` (when documented gaps reveal missing or poorly designed endpoints that warrant a redesign), `/review-code` (reviewing the handlers that were read during discovery), and `/threat-model` (the coverage report surfaces unauthenticated endpoints that warrant security review).
