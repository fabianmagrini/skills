...
- This skill pairs naturally with `/research-codebase` (when a full documentation file is needed rather than an inline explanation), `/map-api-flow` (when the user needs to visualize how API calls flow through the code), and `/onboard-codebase` (when a new engineer needs a complete structured guide rather than a quick answer).
